...
def policy_nn(state, history, state_dim, history_dim, action_dim, initializer):
	# wh1 = tf.get_variable('Wh1', [history_dim, 100], initializer=initializer, regularizer=tf.contrib.layers.l2_regularizer(0.01))
	# bh1 = tf.get_variable('bh1', [100], initializer=tf.constant_initializer(0.0))
	# hh1 = tf.nn.relu(tf.matmul(history, wh1) + bh1)
	# w1 = tf.get_variable('W1', [state_dim+100, 512], initializer = initializer, regularizer=tf.contrib.layers.l2_regularizer(0.01))
	# b1 = tf.get_variable('b1', [512], initializer = tf.constant_initializer(0.0))
	# h1 = tf.nn.relu(tf.matmul(tf.concat([state,hh1], 1), w1) + b1)
